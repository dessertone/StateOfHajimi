...
namespace StateOfHajimi.Engine.Utils;
...
public static class AssetsManager
{
    ...
    public static string BaseUrl { get; set; } = "StateOfHajimi.Assets";
    ...
    public static async Task InitializeAsync()
    {
        await Task.Run(() =>
        {
            var sheetTasks = new[]
            {
                ("LittleHajimiFactory_blue", "Assets/Factories/LightFactory/LightFactory_sheet.png", 2050, 2050),
                ("LittleHajimiFactory_red", "Assets/Factories/LightFactory/LightFactory-red_sheet.png", 2050, 2050),
                ("GroundTexture", "Assets/GroundTexture.jpg", 1024, 1024),
                ("CrystalCluster", "Assets/CrystalCluster.png", 1024, 1024),
                ("CatStatue", "Assets/CatStatue.png", 1024, 1024),
                ("LittleHajimi_blue", "Assets/Entities/LittleHajimi.png", 256, 279),
                ("LittleHajimi_red", "Assets/Entities/LittleHajimi.png", 256, 279),
                ("RallyFlag", "Assets/Cursors/flag.png", 1024, 1024),
                ("Grass", "Assets/Grass2.jpg", 1024, 1024)
            };
            Parallel.ForEach(sheetTasks, task =>
            {
                LoadSpriteSheet(task.Item1, task.Item2, task.Item3, task.Item4);
            });
            ...
            LoadCursorStyle(CursorType.Attack, "Assets/Cursors/Attack.png", new PixelPoint(220, 220));
        });
    ...
    private static void LoadSpriteSheet(string key, string path, int frameW, int frameH)
    {
        try 
        {
            var uri = new Uri($"avares://{BaseUrl}/{path}");
    ...
    public static SpriteSheet? GetSheet(string key, int teamId = -1)
    {
        if(teamId == -1)
